...
class ComPost(models.Model):
    ...
    liked_users = models.ManyToManyField(CustomUser, related_name='liked_composts', blank=True)
...
class ComComment(models.Model):
    id = models.AutoField(primary_key=True)
    compost = models.ForeignKey(ComPost, on_delete=models.CASCADE, related_name='comcomments', null=True)
